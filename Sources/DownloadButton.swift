...
public class StyleKit: NSObject {
    ...
    public static func drawDownloadCompleteState(frame targetFrame: CGRect = CGRect(x: 0, y: 0, width: 50, height: 50), resizing: ResizingBehavior = .aspectFit, palette: Palette, style: DownloadButtonStyle, downloadedManipulable: CGFloat = 0) {
        // General Declarations
        let context = UIGraphicsGetCurrentContext()!
    ...
    public static func drawCheckState(frame targetFrame: CGRect = CGRect(x: 0, y: 0, width: 50, height: 50), resizing: ResizingBehavior = .aspectFit, palette: Palette, style: DownloadButtonStyle, downloadedManipulable: CGFloat = 0, checkRevealManipulable: CGFloat = 0) {
        //// General Declarations
        let context = UIGraphicsGetCurrentContext()!
...
class DownloadButtonLayer: CALayer {
    ...
    override init(layer: Any) {
        ...
        if let layer = layer as? DownloadButtonLayer {
    ...
    override class func needsDisplay(forKey key: String?) -> Bool {
        ...
        if DownloadButtonLayer.isCompatible(key) {
    ...
    override func action(forKey event: String?) -> CAAction? {
        ...
        if DownloadButtonLayer.isCompatible(event) {
...
public protocol DownloadButtonDelegate {
...
open class DownloadButton: UIButton {
    ...
    @IBInspectable open var style: String = DownloadButtonStyle.iOS.rawValue {

        didSet {

            if style == DownloadButtonStyle.macOS.rawValue {

                self.buttonStyle = .macOS

            } else if style == DownloadButtonStyle.tvOS.rawValue {

                self.buttonStyle = .tvOS

            } else if style == DownloadButtonStyle.watchOS.rawValue {
    ...
    open var delegate: DownloadButtonDelegate?
    open var downloadPercent: CGFloat = 0.0 {
        willSet {

            guard let layer: DownloadButtonLayer = layer as? DownloadButtonLayer else { return }
    ...
    private var buttonStyle: DownloadButtonStyle?

    open var downloadState: DownloadButtonState? {

        willSet {

            guard let newValue: DownloadButtonState = newValue else { return }
    ...
    override open class var layerClass: AnyClass {
        ...
        return DownloadButtonLayer.self
    ...
    init(frame: CGRect, isDownloaded: Bool = false, style: DownloadButtonStyle = .iOS, palette: Palette = Palette()) {
    ...
    override open func draw(_ layer: CALayer, in ctx: CGContext) {
        ...
        guard
            let layer: DownloadButtonLayer = layer as? DownloadButtonLayer,
            let buttonStyle: DownloadButtonStyle = buttonStyle
            else { return }
    ...
    private func updateManipulable(_ layer: DownloadButtonLayer, _ keyPath: String, _ value: CGFloat) {
